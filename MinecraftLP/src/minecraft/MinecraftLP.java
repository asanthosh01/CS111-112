...
public class MinecraftLP {
    ...
    public void put(String name, int count) {
        // compute the starting hash index using the provided hash function
        int index = Math.abs(name.hashCode()) % st.length;
        
        // traverse until a null slot or matching item is found
        while (st[index] != null) {
            // if the item already exists, update its count
            if (st[index].getName().equals(name)) {
        ...
        // index should be the first empty slot now
        st[index] = new Item(name, count);
        slotsFilled++;
        
        // if the load factor is at least 0.5 then double the table size
        if (loadFactor() >= 0.5) {
            resize(st.length * 2);
        }
    }
    ...
    public void delete(String name, int count) {
        // find the item's index using the provided search method
        int index = search(name);
        
        // if its not found, then dont do anything
        if (index == -1) {
            return;
        }
        
        // if deletion count is greater than the current count, then do nothing
        if (st[index].getCount() < count) {
            return;
        }
        
        // if more items remain after deletion, decrement the count
        if (st[index].getCount() > count) {
            st[index].setCount(st[index].getCount() - count);
        } else { // exact count deletion, remove the item
            st[index] = null;
            slotsFilled--;
            // rehash the subsequent items to fix any gaps in the table
            rehash((index + 1) % st.length);
        }
        
        // after deletion, if the inventory is larger than 9 and is at most 25% full,
        // reduce table size by using integer div and make sure the minimum length is 9
        if (st.length > 9 && loadFactor() <= 0.25) {
            int newCapacity = st.length / 2;
            if (newCapacity < 9) {
                newCapacity = 9;
            }
            resize(newCapacity);
