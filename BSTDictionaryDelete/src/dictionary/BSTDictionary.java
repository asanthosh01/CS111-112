...
public class BSTDictionary {
    ...
    private WordNode removeWordHelper(WordNode curr, String word) {
        if (curr == null) {
            return null; // base case (word not found)
        }
    
        // recursive traversal to find the node
        if (word.compareTo(curr.getWord()) < 0) {
            ...
            // case 4 (two children, find inorder successor)
                ...
                curr.setRight(removeWordHelper(curr.getRight(), successor.getWord()));
    
                // replacing current node with successor
